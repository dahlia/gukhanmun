...
/// Returns `true` if `raw_attributes` contains a `class` attribute that
/// includes `class_name` as a whitespace-separated token.
...
/// Returns `true` if `raw_attributes` contains `attr_name`, optionally
/// matching a specific `attr_value`.

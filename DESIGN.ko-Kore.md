...
使用者가 날 屬性에 對하여 提供한 任意의 述語. 엔진 自體는 `lang` 屬性이
무엇인지조차 모른다.
...
起源 글字 表에서 標準 讀音을 찾아 한글로 變換한다. 글字 表는 유니코드
`kHangul` 屬性으로부터 빌드되어 *gukhanmun-core*에 完全 해시 테이블로 내장되어
있다; 約 1萬 個의 漢字를 다루며 바이너리에 約 50KB를 더한다.
...
區間에 對하여 動作한다. 出力은 `Annotated` 토큰이 아니라 純粹 텍스트이다, 後續
...
`MapDictionary`는 使用者가 提供한 個別 項目이나 작은 用語集을 爲한 메모리 內
...
`max_word_chars`에 라티스의 出口 狀態를 爲한 작은 常數를 더한 程度로 制限되며,
普通 數 十 字 내외이다.

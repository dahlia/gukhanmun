...
同音異義 區別·루비 마크업·文體上 理由 等을 위해 原 漢字를 倂記할 수 있다. 變換
對象 텍스트 바깥의 構造나 內容은 손대지 않으며, 韓國語가 아닌 區域이나 保存
對象으로 標識된 區域(코드 블록 等)은 그대로 通過시킨다. 入出力은 可能한 範圍
內에서 스트리밍으로 處理하며, 버퍼링은 連續된 漢字 列의 크기와 選擇的 文脈間
同音異義 區別에 局限된다. 메모리 內 데이터·mmap 基盤·또는 엔진에 不透明한 其他
形態의 結合 可能한 辭典을 받아들인다. 大韓民國 國立國語院의
《標準國語大辭典》에서 抽出한 基本 辭典을 함께 提供한다. Rust에서, Node-API를
通한 Node.js에서, WebAssembly를 通한 브라우저와 Deno에서, 그리고 命令줄에서
使用 可能하다.
...
**리더**(reader)는 入力 形式을 IR로 解析한다. **엔진**(engine)은 漢字를 分割해
**註解**(annotation)를 만든다. **미들웨어**(middleware)들은 註解의 플래그를
調整해 IR 흐름을 加工한다. **렌더러**(renderer)는 註解를 具體的 텍스트나
마크업으로 풀어낸다. **라이터**(writer)는 IR을 目標 形式으로 直列化한다. 어느
한 段階를 交替해도 다른 段階들은 影響을 받지 않으며, 段階 사이의 境界는 메서드
呼出이 아닌 明示的인 데이터 흐름이다.

**스트리밍이 基本이다.** 文書 全體를 對象으로 하는 同音異義 區別처럼 本質的으로
버퍼링이 必要한 動作은 選擇 옵션으로 두고, 基本 設定에서는 連續된 漢字 列(普通
數 個의 글字) 안에서, 그리고 한 텍스트 토큰 內部에서만 버퍼링한다. 덕분에
gukhanmun은 큰 文書나 實時間 스트림을 處理하는 서버 內部에서도 適合하며,
WebAssembly 빌드도 긴 記事의 페이지 內 變換에 充分히 實用的이다.
...
pulldown-cmark 이벤트의 變種, 等)의 內容에 對해서는 아무것도 모른다.
...
엔진은 `is_preserve()`를 「이 `Text` 토큰을 건너뛸 것인가」에 對한 唯一한 判斷
根據로 다룬다. 스코프 스택을 거슬러 各 스코프의 `is_preserve()`를 물어보는
方式이다. HTML 어댑터의 `ScopeData` 具顯은 여러 關心事를 그 한 플래그에 모은다:
`lang` 屬性(祖上으로부터 相續되어 韓國語 述語와 比較됨), 태그名(保存 對象 태그
目錄인 `pre`, `code`, `kbd`, `script`, `style`, `textarea`와 比較됨), 그리고
使用者가 날 屬性에 對해 提供한 任意의 述語. 엔진 自體는 `lang` 屬性이
무엇인지조차 모른다.
...
있는) 어댑터 內部로 밀어 넣음으로써, 엔진은 HTML에 對해 無知할 수 있고 IR에는
...
엔진의 任務는 셋이다: 漢字 列을 單語로 分割하는 일, 그 單語들을 辭典에서 찾는
일, 그리고 그에 따라 註解와 폴백 텍스트를 내놓는 일. 또한 漢字 한 글字를 한글로
寫像하고 必要하면 頭音法則을 適用하는 폴백 音譯機와, 漢字 數字를 設定에 따라
한글이나 아라비아 數字로 變換하는 數字 變換器를 함께 가지고 있다.

### 라티스 分割

漢字 列을 辭典 單語들로 分割하는 일은 엔진의 中核 決定이고, 가장 自然스러워
보이는 算法이 틀린 答을 낸다.
...
`行事場入口`라는 入力에 對해 左→右 最長 一致는 `行事場` + `入口`로 分割하는데,
이는 옳다. 그러나 `行事場所`에 對해 같은 算法은 `行事場` + `所`로 分割하고,
`所`는 글字 單位 폴백으로 넘어간다. `行事` + `場所`로 갈라야 두 部分 모두
辭典으로 덮을 수 있는데도 그렇다.

올바른 算法은 라티스 위에서 動的 計劃法을 돌리는 것이다. 漢字 列의 各 글字 位置
`i`에서, 엔진은 辭典에 그 자리로 始作하는 모든 一致를 묻고, 補助로 글字 한
個짜리 폴백 邊을 함께 考慮한다. 點數 函數가 代案들을 評價하고, 우리는 列의
끝에서 비터비式 逆追跡으로 最高點 分割을 選擇한다.

우리가 쓰는 點數 函數는 簡單하며, 위의 `行事場所` 例示에서 옳은 答을 낸다. 길이
`L`의 辭典 一致는 `BASE_DICT + (L - 1) * LENGTH_BONUS`로 評價되고,
`BASE_DICT`는 어떤 폴백 길이로도 이길 수 없을 程度로 크게 잡는다. 폴백 邊은
`BASE_FALLBACK` 點數로, 0 以上이라서 더 긴 分割이 偶然히 選好되는 일은 없다. 이
規則 下에서 `行事` + `場所`(辭典 一致 둘, 글字 4 個 덮음)는 `行事場` +
`所`(辭典 一致 하나 + 폴백 하나, 글字 4 個 덮음)를 이긴다.

라티스 分割의 費用은 列의 길이와 辭典의 最大 項目 길이의 積에 비례한다. 普通의
韓國語 텍스트에서 連續된 漢字 列은 짧다; 大部分 1–4 글字이고, 10 글字를 넘는
일은 거의 없다. 辭典의 最大 項目 길이도 10 字 內外이다. 따라서 列 하나당 數十
番의 辭典 照會면 充分하며, FST와 CDB 백엔드 모두 마이크로秒 單位로 處理한다.

라티스의 正確度가 必要 없고 列 當 追加 費用을 줄이고 싶은 呼出者를 위해
**eager** 分割 戰略도 옵션으로 提供한다. 基本은 라티스이다.

### 폴백 音譯機

辭典 一致가 글字를 덮지 못할 때, 폴백 音譯機는 그 글字를 內藏된 Unihan 起源
글字 表에서 標準 讀音을 찾아 한글로 變換한다. 글字 表는 유니코드 `kHangul`
屬性으로부터 빌드되어 *gukhanmun-core*에 完全 해시 테이블로 내장되어 있다; 約
1萬 個의 漢字를 다루며 바이너리에 約 50KB를 더한다.
...
첫째, `列`·`律` 規則: 이 글字들이 앞 音節의 받침이 ㄴ이거나 받침이 없을 때
「렬·률」 代身 「열·율」로 미끄러진다. 頭音法則 그 自體는 適用되지 않는
자리이지만, 表記 規約이 그렇게 規定한다. 둘째, 漢字 數字 規則: 두 個 以上의
漢字 數字가 連續되면 한 單語로 읽고, 頭音法則은 첫머리에만 適用하고 內部에는
適用하지 않는다. 두 規則 모두 글字 흐름에 對한 작은 파서로 인코딩되어 있다.
...
스택 基盤 累積으로 解析해 아라비아 數字를 産出하며, `一`이 `十` 앞에서 省略되는
...
區間에 對해 動作한다. 出力은 `Annotated` 토큰이 아니라 純粹 텍스트이다, 後續
렌더러가 따로 保存해 둘 만한 漢字-한글 兩立 情報가 없기 때문이다.


辭典
----

辭典은 「주어진 位置에서 始作하는 一致가 무엇인지 물어볼 수 있는」 모든 것이다.
...
**모든** 一致 集合이 必要하므로, 트레이트는 그 全體를 露出한다.
...
`UnihanCharDict`는 글字 單位 폴백을 `HanjaDictionary`로 露出한 것이다. 엔진의
인터페이스를 統一하기 위함이다; 모든 照會는 `matches_at`을 通하고, 글字 單位
基本 境遇도 例外가 아니다. 具顯은 유니코드 `kHangul` 屬性에서 빌드 時點에
만들어진 完全 해시 테이블이다.

`MapDictionary`는 使用者가 提供한 個別 項目이나 작은 用語集을 위한 메모리 內
辭典이다. 해시맵이 아니라 `fst::Map`을 基盤으로 한다; FST는 順序 接頭辭 巡廻를
마이크로秒 單位로 提供하고 壓縮이 좋아, 特히 WebAssembly 번들에서 重要하다.

`ChainDictionary`는 優先順位 政策에 따라 辭典들을 連續으로 結合한다. 典型的
構成은 작은 使用者 辭典(最高 優先), 主題別 辭典, 《標準國語大辭典》,
`UnihanCharDict`(最低 優先) 順이다.
...
各各에 對해 한 個의 레코드를 저장하며, 完全한 單語인지 中間 接頭辭인지를 1
바이트 플래그로 區分한다:
...
使用者의 優先順位가 다르기 때문이다; CDB는 코드 監査와 사소한 mmap 支援을 위한
選擇이고, FST는 작은 WebAssembly 번들을 위한 選擇이다.
...
들고 있다. 플래그는 엔진이 該當 註解에 對해 알고 있는 것을 描寫한다: 辭典에서
...
언제 리셋되는지」와 같은 政策을, 「괄호·루비·한글 專用」 같은 形式 決定과
...
`FirstOccurrenceFilter`는 設定된 文脈 內에서 첫 登場 以後의 註解들에 對해
...
헤딩 스코프의 `is_block_boundary()`로 그 境界를 露出한다.
...
클로저일 수 있다; JavaScript 側에서는 토큰 當 境界 橫斷 費用을 避하기 위해
文字列 集合 形態만 露出한다.
...
모드로 決定된다: `on-hangul`은 `<ruby>한글<rt>漢字</rt></ruby>`, `on-hanja`는
`<ruby>漢字<rt>한글</rt></ruby>`. 現在 스코프의 `allows_inline_markup()`이
false면 괄호 形式으로 還元한다.

`Original` 렌더러는 原 漢字를 純粹 텍스트로 내보낸다. `require_hangul`이
設定되어 있거나 使用者 指示로 標識된 註解만 倂記를 받으며, 倂記 形態는 下位
옵션에 따라 괄호나 ruby가 된다. 「國漢文 表記는 그대로 두고 어려운 글字만
...
單獨이면 1 個, 괄호 形式이면 3 個, ruby면 5–9 個). 狀態도 없고 버퍼링도 없다.
...
가지든가, 렌더러가 다른 미들웨어들에 對해 알고 있어야 하는 結果가 된다.
...
아니라 AST를 産出한다; 스트리밍 性質을 維持하기 위해 이벤트 흐름을 選好했다.
...
값에 對해서는 `invalid-input` 코드의 `GukhanmunError`를 던진다.
...
엔진 內部에서는 청크가 漢字 列 中間에서 끝나면 그 漢字 列의 꼬리를 다음
청크까지 保留하며, 그 앞까지는 즉시 flush한다. 꼬리 버퍼는 辭典의
`max_word_chars`에 라티스의 出口 狀態를 위한 작은 常數를 더한 程度로 制限되며,
普通 數 十 字 내외이다.

스트림 타입으로 `Uint8Array`가 아니라 文字列을 擇한 理由는 엔진이 根本的으로
유니코드 스칼라 단위로 動作하기 때문이다. 바이트 單位 청크화는 어댑터가 모든
...
버전은 올라간다. 言語 間 이야기를 明確하게 維持하기 위해서이다. 패키지 個別
...
`tracing`으로 로그하고 該當 區間에 對해 `Verbatim` 토큰을 내보내, 그 뒤
토큰들이 繼續 흐르게 한다.
...
**性質 基盤 테스트**는 `proptest`로 生成된 入力에 對해 不變式을 主張한다. 가장
...
**適合性 테스트**는 CommonMark 仕樣의 一部 例示에 對해 Markdown 어댑터를 돌려,
...
分割, 폴백 區間에 適用되는 頭音法則, 段落 內에서 讀音이 曖昧할 때 漢字를 괄호로
...
基本 `HtmlScopeData::is_preserve()`는 屬性 內容과 無關하게 다음 태그名에 對해

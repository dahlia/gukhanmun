...
래티스(lattice) 基盤 分割, 더 多樣한 出力 形式. 프로젝트는 Rust로 具顯되었고
Rust 라이브러리·命令줄 道具·WebAssembly 바인딩·[Node-API] 바인딩의 形態로
提供된다.
...
않는다; 意味 保存은 約束하지만 原形 保存은 最善의 努力일 뿐이다. 言語 間
...
엔진은 **形式 中立的**이다. 같은 엔진이 HTML·Markdown·平文 텍스트를 모두 處理할
...
德分에 한 番 만든 테스트 픽스처가 세 形式 모두에서 意味를 가지게 되고, 새
形式을 追加하는 作業이 局所化된다.
...
動作은 該當 文脈 境界까지 버퍼링한다. HTML과 Markdown에서는 基本 블록 單位
同音異義 窓이 普通 段落·리스트 項目·헤딩 같은 스코프에서 境界를 만난다. 反面
平文 텍스트에는 블록 스코프가 없으므로 같은 基本 窓이 文書 全體 窓이 된다:
나중 줄의 註解가 앞 줄의 註解에 漢字 倂記를 要求할 수 있고, 바이트 스트림은 이미
標準 出力에 쓴 텍스트를 되돌려 고칠 수 없다. 平文 텍스트에서 즉시 出力이 必要한
呼出者는 同音異義 標識를 끄고, 그에 따른 區別 情報 損失을 받아들여야 한다.

스트리밍 變換은 폴백(fallback) 註解 範圍도 正確하게 維持한다. 꼬리에 남은 폴백
漢字 連續 範圍는 辭典의 最大 單語 길이보다 길더라도, 後續 非變換 境界나 EOF를
만날 때까지 保留한다. 이 때문에 폴백 漢字만 이어지는 入力은 辭典 先讀만 놓고
보면 必要한 것보다 덜 卽時的일 수 있지만, `hangul-hanja-parens` 같은 렌더링
모드가 單發(one-shot) 變換과 같은 註解 묶음을 보게 된다.
...
엔진에 들어가는 토큰은 다음 中 하나이다:
 ...
 -  `Verbatim(Cow<str>)`: HTML의 `<code>` 內容이나 Markdown 코드 스팬처럼,
    그대로 通過되어야 하는 텍스트. 무엇이 그대로 通過되어야 하는지는 엔진이
    아니라 어댑터가 決定한다.
...
올바른 算法은 래티스 위에서 動的 計劃法을 돌리는 것이다. 變換 候補 範圍의 各
글字 位置 `i`에서, 엔진은 辭典에 그 자리로 始作하는 모든 一致를 묻고, 現在
글字가 漢字일 때에는 補助로 글字 한 個짜리 폴백 邊을 함께 考慮한다. 評價函數가
代案들을 比較하고, 우리는 範圍의 끝에서 비터비(Viterbi)式 逆追跡으로 最適 分割을
選擇한다.
...
래티스 分割의 費用은 變換 候補 範圍의 길이와 辭典의 最大 項目 길이의 곱에
...
래티스의 正確度가 必要 없고 範圍 當 追加 費用을 줄이고 싶은 呼出者를 爲하여
**eager** 分割 戰略도 옵션으로 提供한다. 基本은 래티스이다.
...
數字 變換은 폴백 經路 안에서, 래티스가 「辭典에 一致하지 않음」으로 判定한
...
正規化를 出力하는 것이므로, 平文 텍스트를 내보낼 수 있다.
...
排除한 算法이다. 래티스 分割機가 代案들을 評價하려면 始作 位置에서 가능한
...
폴백(fallback)하며 文脈 內 표시도 계속 받는다. 窓은
...
平文 텍스트에는 블록 스코프가 없으므로 `per-block`은 文書 全體 窓이 된다. 이는
...
`Original` 렌더러는 原 漢字를 平文 텍스트로 내보낸다. `require_hangul`이
...
平文 텍스트 어댑터는 全體 入力을 單一 스코프로 감싸 한 個의 `Text` 토큰을
내보낸다. 出力은 `Text` 토큰들의 連結이다. Ruby 렌더링은 平文 텍스트에서 意味가
없으므로 括弧 形式으로 還元된다. CLI는 이미 렌더링한 出力을 文書 全體
미들웨어가 나중에 바꿀 可能性이 없을 때에만 平文 텍스트를 EOF 前에 스트리밍할 수
있다. 實際로 `ko-kp` 프리셋은 同音異義 標識이 꺼져 있으므로 스트리밍하지만,
基本 `ko-kr` 프리셋은 줄 사이 同音異義를 正確히 렌더링하기 爲해 平文 텍스트
出力을 EOF까지 保留한다.
...
Rust 코드는 Cargo 워크스페이스로 다음과 같이 構成된다:
 ...
 -  *crates/*
     -  *gukhanmun-core/*: IR 타입·엔진·辭典 트레이트·래티스 分割機·폴백
...
辭典의 `max_word_chars`에 래티스의 出口 狀態를 爲한 작은 常數를 더한 程度로
制限되며, 普通 數 十 字 내외이다. 그러나 폴백 漢字만 이어지는 連續 範圍는
...
同音異義 標識, 그리고 블록 스코프가 없어 文書 全體처럼 動作하는 平文 텍스트
...
`ko-kr` 프리셋은 大韓民國의 正書法과 語彙 慣行에 맞춘다: 辭典 主導 讀音, 래티스

...
엔진에서 나오는 토큰은 다음 中 하나이다:
 ...
 -  `Annotated(Annotation)`: 엔진이 한 漢字語를 變換한 자리. 註解는 原 漢字,
    一致한 境遇의 辭典 標準 表記, 한글 讀音, 그리고 이 變換이 왜 일어났고 後續
    段階가 그것을 어떻게 다룰지를 描寫하는 플래그를 들고 있다.
...
`dictionary_hanja`는 辭典 基盤 註解이면 `Some`, 폴백 註解이면 `None`이다.
異體字 入力은 `hanja`에 原文을 保存하고, 미들웨어는 辭典 表記를 語彙 識別子로
使用하며, 렌더러는 이를 異體字 集合의 入力으로 使用한다.
...
글字가 漢字일 때에는 補助로 글字 한 個짜리 폴백 邊을 함께 考慮한다. 아라비아
數字 戰略은 그 位置에서 數字가 始作될 때 平文 數字 邊도 追加한다. 評價函數가
代案들을 比較하고, 우리는 範圍의 끝에서 비터비(Viterbi)式 逆追跡으로 最適
分割을 選擇한다.

評價函數는 意圖的으로 單純하다. 먼저 폴백에 남는 글字 數를 最小化한다. 따라서
`行事` + `場所`는 폴백을 남기지 않으므로 `行事場` + `所`보다 낫다. 폴백으로
남는 글字 數가 같으면, 設定된 數字 戰略이 아라비아 數字 邊을 만들 수 있을 때 그
邊을 辭典 一致보다 選好한다. 그래서 `六月` 같은 辭典 曆法 項目도 `6월`로
正規化될 수 있다. 그 다음에는 辭典으로 덮은 글字 數를 最大化하고, 다시 같으면
分割 數가 더 적은 쪽을 選好한다. 그래서 `天地` 같은 全體 單語 一致는 `天` + `地`
같은 構成要素 分割보다 낫다. 그래도 同點이면 같은 點數에 먼저 到達한 候補를
維持해 結果를 決定的으로 만든다.
...
基本값이다. 命令줄 道具는 `--numerals`가 省略될 때 別途의 基本값으로
`smart`를 使用한다. 命令줄 出力은 보통 사람이 읽는 最終 렌더링이기 때문이다.
`positional-arabic` 戰略은 數字만으로 이뤄진
...
文脈을 본다: 數字로 始作하는 加算 數詞에는 `additive-arabic`을 쓰고,
`十`·`百`·`千` 같은 작은 자릿값 標識로 始作하는 列은 다음 글字가 애매한
非單位 漢字 單語 글字가 아닐 때만 `additive-arabic`을 쓴다. 4 字 以上의 純粹
數字이거나 單位 漢字(`年月日時分秒號世紀` 等)가 뒤따르는 純粹 數字에는
`positional-arabic`을 쓴다(年度 表記 慣行에 맞춤). `萬`이나 `京` 같은 큰
자릿값 標識가 單獨으로 나오거나, `百濟`·`十長生` 같은 작은 자릿값 標識
始作 單語인 境遇에는 `hangul-phonetic`으로 還元하여 未知 單語가 數字로 쪼개지지
않게 한다.

`hangul-phonetic` 戰略은 폴백 經路 안에서, 래티스가 「辭典에 一致하지 않음」으로
判定한 區間에 對하여 動作한다. 이 戰略은 폴백 `Annotated` 토큰을 내보내어 原
漢字 數字를 保存하므로, `HangulHanjaParens` 같은 렌더러도 原文을 表示할 수
있다. 아라비아 數字 戰略은 平文 텍스트 數字 邊으로 分割에 參與하므로, `六月`과
`十月` 같은 辭典 曆法 項目보다 數字 正規化가 優先될 수 있다. 이런 代替가 數字
뒤에 認識된 單位 漢字만 붙은 辭典 一致에 걸릴 때는 平文 邊이 그 suffix까지 덮고
폴백 文脈으로 읽는다. 따라서 `月` 單獨 項目이 없어도 `六月`은 `6월`로
正規化될 수 있다. 數字 邊은 數字 列의 始作에서만 始作하므로,
`positional-arabic`은 `十一月` 같은 加算 表記를 꼬리의 `一`만 變換하는 方式으로
쪼개지 않는다. 數字와 認識된 單位 漢字를 넘어 더 길게 이어지는 辭典 一致는
여전히 그 單語를 保護하므로, `smart`는 `北京`이나 `一分錢` 같은 普通 辭典 項目을
쪼개지 않는다.
...
異體字 認識은 언제나 켜져 있으며 `HanjaDictionary` trait나 FST/CDB 形式을
變更하지 않는다. 엔진은 各 原文 位置에서 먼저 正確한 辭典 接頭辭를 照會한다.
같은 길이에 正確 一致가 없으면 各 原文 글字에서 出力 異體字 集合과 같은 方向性
있는 互換·常用漢字·傳統·簡化·朝日文字 投影으로 制限된 表記 集合을 만든다.
逆方向 關係는 正確한 對象字의 原字가 하나일 때만 使用한다. 엔진은 이 글字別
表記의 直積을 列擧하며, 生成한 表記도 辭典의 完全한 키일 때만 採擇하므로 辭典이
判定 基準으로 남는다. `芸`의 直接 一致는 같은 길이의 擴張만 막으므로, 原文
`芸術`에서 더 긴 `藝術` 一致를 가리지 않는다.

生成 方向 매핑 表는 Unicode 17.0.0의 `kZVariant`, `kSimplifiedVariant`,
`kTraditionalVariant`, `kCompatibilityVariant`, 日本 文化廳의 常用漢字 新舊
字體 쌍, 그리고 檢證한 열여섯 朝日文字 쌍을 使用한다. 意味·特殊 意味·欺瞞
關係는 除外한다. 對象字가 여러 個인 Unicode 關係를 同値 類로 따라가지 않으므로,
같은 簡化字를 通해서만 만나는 글字들은 서로 交換되지 않는다. 自體 매핑이 아닌
簡化 關係는 그 原字가 다른 매핑의 對象字이더라도 方向 關係로 保存한다. 接頭辭는
32글字, 直積 候補는 256個로 制限한다. 글字別 表記 集合은 各 照會 窓에서 한 번만
計算하며, 生成한 逆方向/對象字 인덱스로 모든 表 照會를 對數 時間에 處理한다.
限度를 넘으면 그 接頭辭 길이의 擴張만 건너뛴다. 完全히 같은 重複을 除去한 뒤,
完全한 辭典 一致가 正確히 하나만 남을 때에만 異體字 一致를 採擇한다. 讀音이
같더라도 辭典 標準 表記나 一致 메타데이터가 다르면 原文만으로 單語의 正體를
確定할 수 없으므로 曖昧한 一致로 보고 모두 버린다.

互換 漢字 폴딩은 글字 單位 폴백 讀音에도 適用되어 `神`은 `神`의 讀音을
繼承한다. 讀音 照會는 KS X 1001/1002에서 물려받은 區別을 包含하여 互換
코드포인트에 直接 配定된 `kHangul` 값은 意圖的으로 無視한다. Gukhanmun은 이런
코드포인트를 獨立된 語彙 文字가 아니라 統合 漢字의 表現 異體로 다룬다. 括弧 讀音
檢證은 原文 基準을 維持한다.
...
모든 모드는 漢字를 내보낼 때 獨立된 `HanjaVariantSet`을 適用한다.
基本 `AsDictionary`는 `dictionary_hanja`를 保存하고, 폴백 註解는 原文을
保存한다. `Shinjitai`는 常用漢字 新字體를, `Kanxi`는 互換 폴딩한 傳統 字體를,
`Simplified`는 中國 簡化字 代表를, `Asahimoji`는 新字體와 檢證한 朝日文字
集合을 使用한다. 이 變換은 方向이 있는 原字/對象字 關係를 合成한다. `Kanxi`는
서로 連結된 각 `kZVariant` 컴포넌트에서 Unicode 스칼라 값이 가장 작은 글字를
決定的 代表로 選擇하므로, 相互 매핑도 安定的이고 冪等인 出力을 만든다. 傳統字
投影 뒤에 互換 폴딩을 다시 適用하므로 方向 매핑이 互換 漢字를 되살리지 않는다.
逆方向 關係는 該當 對象字의 原字가 正確히 하나일 때만 使用하므로, 하나의
簡化字를 共有할 뿐인 다른 意味 사이로 字體가 넘어가지 않는다. 要請한 集合의
直接 매핑은 다른 集合을 거치는 投影보다 먼저 適用하며, 이미 常用漢字나
朝日文字 對象字인 글字는 그 集合에 그대로 남긴다. 簡化字 出力의 다른 集合을
거치는 逆投影은 直接 簡化 매핑에 到達할 때만 採擇하고, 그렇지 않으면 現在 글字를
그대로 保存한다. 이는 出力 政策일 뿐이며 모든 集合은 같은 認識과 讀音을
共有한다.

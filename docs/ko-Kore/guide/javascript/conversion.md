...
記數法을 고르되 `百濟`나 `十長生`처럼 單語처럼 보이는 애매한 列은 音讀
폴백으로 둡니다.
...
앞에(`곳간(庫間)`) 달고 있으면, Gukhanmun은 基本的으로 重複된 括弧를 除去하고
그 單語를 두 文字 體系로 보여 줍니다(`곳간(庫間)`).  代案 讀音을 固定하는 括弧는
그 자리의 辭典 讀音을 덮어쓰므로, `數字`가 普通 `숫자`로 읽히더라도
`數字(수자)`는 `수자(數字)`가 됩니다.
...
import { load } from "@gukhanmun/wasm";
// ---cut-before---
const g = await load({ collapseRedundantParens: true });   // 基本
const g = await load({ collapseRedundantParens: false });  // 그대로 둠
~~~~
...
`庫間(물건을 간직하여 두는 곳)`이나 外來語 音譯 `蔣介石(장제스)`는 두 規則
어디에도 該當하지 않아 건드리지 않습니다.
